...
To asses the worth of our algorithm we considered as main metrics the Bit Error Rate (BER) and the average throughput of involved users and compared the proposed optimization with other 2 ABS choice policy in 3 different traffic scenario.
\begin{itemize}
	\item[Static ABS]: the number of Almost Blank Subframes remains constant to 4 over the entire simulation and  cells use the same ABS mask 
    [0,1,0,1,0,1,0,1,0,0]
    \item[Random ABS]:  at each frame macro cell randomlY selects the ABS mask choosing over three possible action:increae, decrease or keep constant the number of almost blank subframes. Each 
\end{itemize}
}
\bgroup
...
\begin{footnotesize}
	\begin{table}[t]
		\centering
		\vspace{10pt}
		\caption{Adaptation algorithm parameters}
		\label{table:params}
		\begin{tabular}{lcc} 
			\hline
			\multicolumn{1}{l|}{\scshape ABS policy} & \multicolumn{1}{c}{\scshape Parameter}   	& \scshape Value    \\ \hline
			\multicolumn{1}{l|}{Random policy}       & \multicolumn{1}{c|}{$P_{increasing}$} 		& 5                 \\ 
			\multicolumn{1}{l|}{}                   	& \multicolumn{1}{c|}{$P_{constant}$} 			& 3\\
			\multicolumn{1}{l|}{}                   	& \multicolumn{1}{c|}{$P_{decreasing}$} 		& 3       \\ \hline
			\multicolumn{1}{l|}{Static}				& \multicolumn{1}{c|}{$N_{ABS}$} 	 			& 30 s		     \\         
...
\textcolor{red}{The learning algorithm was trained over \textbf{N} different simulations composed of 125 frames of 10 ns for a total of \textbf{N*125} frames and simulated time \textbf{N*125*10ns} seconds.
Using the off-policy propery of the Q-learning algorithm we ensured that all possible states were visited several times such that the value function for the pair state action could be well approximate.
The generated traffic used}
